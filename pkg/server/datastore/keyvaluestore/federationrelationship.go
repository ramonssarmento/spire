...
func (ds *DataStore) ListFederationRelationships(ctx context.Context, req *datastore.ListFederationRelationshipsRequest) (*datastore.ListFederationRelationshipsResponse, error) {
	records, cursor, err := ds.federationRelationships.List(req)
	if err != nil {
		return nil, err
	}
...
type federationRelationshipIndex struct {
	all record.Set[federationRelationshipObject]
}

func (c *federationRelationshipIndex) Count() int {
	return c.all.Count()
}

func (c *federationRelationshipIndex) Get(key string) (*record.Record[federationRelationshipObject], bool) {
	return c.all.Get(key)
}

func (c *federationRelationshipIndex) Put(r *record.Record[federationRelationshipObject]) error {
	c.all.Set(r)
	return nil
}

func (c *federationRelationshipIndex) Delete(key string) {
	c.all.Delete(key)
}

func (c *federationRelationshipIndex) List(req *datastore.ListFederationRelationshipsRequest) (record.Iterator[federationRelationshipObject], error) {
